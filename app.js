...
function initData() {
  ...
  Object.keys(defaults).forEach(function (k) {
    ...
    STORE[k] = (loaded !== null && loaded !== undefined) ? loaded : defaults[k];
  });
}
...
function renderDev() {
  ...
  document.getElementById('pg-dev').innerHTML =
  ...
  document.getElementById('refl-btn').onclick = function () {
    STORE.refl = document.getElementById('refl-ta').value; save('refl');
...
function renderPrayers() {
  ...
  pf.innerHTML = '';
  ['Todas'].concat(STORE.pcats).forEach(function (c) {
  ...
  items.forEach(function (p) {
    ...
    d.innerHTML =
      ...
      + '<div style="flex:1"><div style="font-size:13px;text-decoration:' + (p.done ? 'line-through' : 'none') + ';color:' + (p.done ? '#a06090' : '#fce4ff') + '">' + esc(p.text) + '</div>'
      + '<div style="font-size:10px;color:#e91e8c">' + esc(p.cat) + ' · ' + esc(p.date) + '</div></div>'
  ...
  list.querySelectorAll('.del-p').forEach(function (b) {
    b.onclick = function () {
      ...
      STORE.prayers = STORE.prayers.filter(function (p) { return p.id !== Number(b.dataset.id); });
      save('prayers'); renderPrayers();
    };
  });
}
...
function openPrayerModal(id) {
  ...
  document.getElementById('m-psave').onclick = function () {
    ...
    if (p) { STORE.prayers = STORE.prayers.map(function (pp) { return pp.id === id ? Object.assign({}, pp, { text: t, cat: c }) : pp; }); }
    else   { STORE.prayers.unshift({ id: uid(), text: t, cat: c, done: false, date: tod() }); }
...
function renderAgenda() {
  ...
  var dayBtns = DIAS.map(function (d, i) {
    ...
    return '<button class="btn day-btn" data-d="' + i + '" style="flex:1;padding:8px 2px;font-size:12px;position:relative;background:' + (selDay === i ? '#e91e8c' : '#1a0018') + ';color:' + (selDay === i ? '#fff' : '#a06090') + ';border-color:' + (selDay === i ? '#e91e8c' : '#3d0035') + '">' + d + (has ? '<span style="position:absolute;top:3px;right:3px;width:5px;height:5px;border-radius:50%;background:#ffd700;display:block"></span>' : '') + '</button>';
  }).join('');
  ...
  pg.innerHTML =
    ...
    + '<button id="btn-mat-mgr" class="btn btns">⚙️ Materias</button></div></div>'
    + '<div id="ev-list"></div>';
...
function renderEvents() {
  ...
  if (!evs.length) { list.innerHTML = '<div class="card" style="text-align:center;color:#a06090">Día libre 🌸</div>'; return; }
  list.innerHTML = '';
  evs.forEach(function (ev) {
    ...
    d.innerHTML =
      ...
      + '<div style="flex:1"><div style="font-size:13px;text-decoration:' + (ev.done ? 'line-through' : 'none') + '">' + esc(ev.title) + '</div>'
      + '<div style="font-size:10px;color:' + mat.color + '">' + esc(ev.mat) + (ev.hora ? ' · ' + esc(ev.hora) : '') + '</div></div>'
  ...
  list.querySelectorAll('.del-e').forEach(function (b) {
    b.onclick = function () {
      if (!confirm('¿Eliminar?')) return;
      STORE.events = STORE.events.filter(function (e) { return e.id !== Number(b.dataset.id); });
...
function openEvModal(id) {
  ...
  showModal('<h3>' + (ev ? 'Editar evento' : 'Nuevo evento — ' + DIAS[selDay]) + '</h3>'
  ...
  document.getElementById('m-esave').onclick = function () {
    var t = document.getElementById('m-et').value.trim(); if (!t) return;
    var h = document.getElementById('m-eh').value.trim();
    var m = document.getElementById('m-em').value;
    if (ev) { STORE.events = STORE.events.map(function (e) { return e.id === id ? Object.assign({}, e, { title: t, hora: h, mat: m }) : e; }); }
    else    { STORE.events.push({ id: uid(), day: selDay, title: t, mat: m, hora: h, done: false }); }
...
function openMatModal() {
  ...
  document.getElementById('m-matadd').onclick = function () {
    STORE.materias.push({ name: 'Nueva materia', color: '#a06090' });
    openMatModal();
  };
  document.getElementById('m-matsave').onclick = function () {
